##basic
1. 4style \displaystle \textstyle \scriptstyle \scriptscriptstyle
2. $$$a_{2}+a^{2}+\_2a$$$
3. $$$\frac{x}{y}$$$ 
4. $$$\sum\_{i}^{n}+\int_{a}^{b}$$$,max,gcd,log,sup,lim
5. $$$\int\\!\\!\\!\int_{D_n}w(z)dz$$$
6. $$$\int\int_{D_n}w(z)dz$$$
7. $$$\int\\!\\!\\!\int\limits_{\\!D_n}w(z)dz$$$
8. $$$\prod_{j=0,j\ne i}^{n}$$$
9. $$$\prod\limits_{j=0,j\ne i}^{n}$$$
10. $$$\sqrt[n]{a}$$$
11. $$$\left[\frac xy\right]+\left|\frac xy\right|+\left\\{\frac xy\right\\}+\left\\|\frac xy\right\\|$$$
12. $$$\mbox{this is }:\frac xy$$$
13. $$$x\not < 1 or x\stackrel{def}{=}$$$
14. eqnarray
\begin{eqnarray*}
y &= &a label{eq:2}\\\
  &= &b \\\
...
15. mathbf,partial $$$\mathbf{V}_1 = \dot{y}\frac{\displaystyle \partial x}{\displaystyle \partial y}$$$

16. vmatrix ,pmatrix,bmatrix,Bmatrix
